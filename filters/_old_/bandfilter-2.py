...
def create_notched_music(input_file, output_file, lower_freq, upper_freq, intermittent=False, interval=1.0):
    try:
        print(f"Processing file: {input_file}")
        print(f"Filter range: {lower_freq} Hz - {upper_freq} Hz")
        ...
        # Ensure upper_freq is not above Nyquist frequency
        upper_freq = min(upper_freq, sample_rate/2 - 1)
        print(f"Adjusted filter range: {lower_freq:.2f} Hz - {upper_freq:.2f} Hz")
        ...
        if intermittent:
            ...
            for i in range(0, len(data), chunk_size):
                ...
                if np.mean(sine_wave[i:i+chunk_size]) > 0.5:
                    filtered_chunk = apply_notch_filter(chunk, lower_freq, upper_freq, sample_rate)
                else:
            ...
            filtered_data = apply_notch_filter(data, lower_freq, upper_freq, sample_rate)
...
lower_freq = 9996  # Your tinnitus frequency
upper_freq = 15000  # Upper limit of the filter
intermittent = True  # Set to False for continuous filtering
interval = 2.0  # Interval for intermittent filtering in seconds

start_time = time.time()
create_notched_music(input_file, output_file, lower_freq, upper_freq, intermittent=intermittent, interval=interval)
